...
export function GraphVisualization({
  ...
  const nodeCanvasObject = useCallback(
    (node: any, ctx: CanvasRenderingContext2D, globalScale: number) => {
      ...
      if (globalScale > 0.8 && isVisible) {
        ...
        if (globalScale > 1.2) {
          const nameAttr = graphNode.properties?.Name || graphNode.properties?.name;
          const descAttr = graphNode.properties?.Description || graphNode.properties?.description;
  ...
  const linkCanvasObject = useCallback(
    (link: any, ctx: CanvasRenderingContext2D) => {
      ...
      // Get colors from source and target nodes for edge
      const sourceColor = (sourceNode.properties?._schemaColor as string) || NODE_COLORS[sourceNode.type] || '#3b82f6';
      const targetColor = (targetNode.properties?._schemaColor as string) || NODE_COLORS[targetNode.type] || '#3b82f6';

      ctx.beginPath();
      ctx.moveTo(sourceX, sourceY);
      ctx.lineTo(targetX, targetY);
      ctx.strokeStyle = isVisible ? '#333' : '#33330';
      ...
      if (isVisible) {
        ...
        // Draw relationship label on edge (rotated to follow edge direction)
        const midX = (sourceX + targetX) / 2;
        const midY = (sourceY + targetY) / 2;
        const labelangle = Math.atan2(targetY - sourceY, targetX - sourceX);
        
        const relationshipType = (link as any).relationshipType || (link as any).type || (link as any).label || '';
        if (relationshipType && isVisible) {
          ...
          const relationshipMap: Record<string, string> = {
            ...
            'CONTAINEDINSPATIALSTRUCTURE': 'HasContained',
            'VOIDSELEMENT': 'FillsVoids',
            ...
            'HASSPACEKEYRELATIONSHIPTOSPATIALELEMENT': 'HasSpaceBoundary',
            'RELATESTOSPATIALELEMENT': 'HasOpenings',
            'RELATESGEOMETRICDATA': 'Geometry',
            'CONNECTEDTO': 'HasConnected',
          };
